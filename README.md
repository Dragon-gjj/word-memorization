...
- 若你需要持久化 AI 配置（而不是仅本次会话），可以将配置保存到磁盘（加密后）或读取系统环境变量。
- 如需支持更多国内厂商（或不同的认证头格式），建议在 `call_qianwen` 中添加对 `X-API-KEY` 等头字段的兼容分支，或在 UI 中增加一个下拉以选择密钥放置方式。

---

如需我把 README 的其他语言版本、或把使用说明拆分为教程页面（含截图和操作流程 GIF），我可以继续帮你生成。欢迎告诉我你想要的格式或补充内容。